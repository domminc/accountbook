...
휴대폰에서는 아래 메뉴, PC(폭 1024px 이상)에서는 왼쪽 메뉴와 넓은 화면으로 보인다. PC의 거래 내역은 표로 보이고, 표 위 한 줄 입력(날짜·소분류·금액·지출방법·내용 → Enter)으로 시트처럼 이어서 입력한다.

| 메뉴 | 내용 |
|---|---|
| 홈 | 이달의 정리: 남은 금액·수입·지출·저축·저축률, 목표 달성률, 대분류별 예산, 저축·지출 비중, 무지출 Day, 결제 수단별·태그별, 카드별 사용(예산 대비), 과소비 알림, 이달의 이벤트 |
| 내역 | 월별 거래 목록·달력, 검색·필터, 입력·수정·삭제, 영수증 사진, 카드 문자 붙여넣기·자동 입력, 지난달 고정지출 가져오기 |
| 예비비 | 연도별 분류·입금·지출·잔액 |
| 연간 | 수입·저축·지출·예비비 월별 상세표, 월평균 |
| 설정 | 구성원·초대, 문자 자동 입력(아이폰 단축어·안드로이드), 자산·대출·카드·통장·결제일, 카테고리·지출방법·태그, 데이터 가져오기·내보내기 |
| (홈에서) | 목표·예산·과소비 한도 입력, 주간별 표 |

> 배우자는 **설정 → 구성원·초대**에서 만든 초대 링크로 같은 가계부에 들어온다. 링크 없이 가입하면 따로 가계부가 만들어진다.

### 오프라인 입력

한 번 연결된 상태로 앱을 열면 거래 입력 화면을 기기에 받아 둔다. 지하철처럼 연결이 없을 때 앱을 열면 입력 화면이 열리고, 저장한 거래는 기기에 모아 두었다가 연결되면 자동으로 올린다 (화면 위 안내에 남은 건수 표시). 서비스 워커는 `public/sw.js`, 대기열은 `src/lib/offline-queue.ts`, 올리기는 `src/components/offline-sync.tsx`.
